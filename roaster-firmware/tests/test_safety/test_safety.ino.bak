...
test(Safety_ThermalRunaway_RampingSetpoint) {
  ...
  // May not trigger immediately, but should be detected
  assertTrue(millis() - testLastTempIncrease >= stableTime);
}
...
test(Safety_OverTemp_Recovery) {
  ...
  if (testCurrentTemp < MAX_SAFE_TEMP - 50.0) { // Hysteresis
    testSafetyShutdown = false;
  }
...
test(Safety_SensorFault_ShortedThermocouple) {
  ...
  if (tempChange > 100.0) { // Impossible 100°F instant drop
    testSensorFault = true;
    testHeaterOutput = 0;
    testFanSpeed = 255;
  }
...
test(Safety_SensorFault_UnrealisticJump) {
  ...
  if (rateOfChange > 200.0) { // Max realistic rate
    testSensorFault = true;
    testHeaterOutput = 0;
  }
...
test(Safety_EmergencyShutdown_AllConditions) {
  ...
  testCurrentTemp = 510.0;
  if (testCurrentTemp > MAX_SAFE_TEMP) overTemp = true;
  
  testCurrentTemp = 999.0;
  if (testCurrentTemp > SENSOR_FAULT_THRESHOLD) sensorFault = true;
